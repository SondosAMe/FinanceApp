...
function updateDashboard() {
    ...
    const monthlySavings = calculateMonthlySavings(month);
    const totalSavings = calculateTotalSavings(month);
    const carryover = getCarryoverSavings(month);
    ...
    // Show total savings (with carryover) in the savings display
    document.getElementById('savingsDisplay').textContent = formatCurrency(totalSavings);
    
    // Update savings color
    const savingsEl = document.getElementById('savingsDisplay');
    savingsEl.className = totalSavings >= 0 ? 'metric-value savings positive' : 'metric-value savings negative';
    
    // Add tooltip or title to show breakdown if there's carryover
    if (carryover > 0) {
        savingsEl.title = `Total: ${formatCurrency(totalSavings)} (This month: ${formatCurrency(monthlySavings)}, Carryover: ${formatCurrency(carryover)})`;
    } else {
        savingsEl.title = `This month: ${formatCurrency(monthlySavings)}`;
    }
}
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // Use calmer purple/indigo gradient for dark mode
        const baseColor = `rgba(124, 58, 237, ${0.2 + opacity * 0.6})`;
        html += `<div class="calendar-day clickable" data-date="${dateStr}" style="background-color: ${baseColor};" title="${dateStr}: ${formatCurrency(spending)}">`;
